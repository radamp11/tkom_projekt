...
LexicalAnalyzer::LexicalAnalyzer(const std::string& fileName) {
    ...
    catch (const std::string& eMessage) { std::cerr << eMessage << std::endl; }
    lineNumber = 1;
    currentPosition = 0;
...
bool LexicalAnalyzer::isComment() {
    if (currentChar == '/') {
        ...
        if (nextChar == '/') {
            currentChar = getNextChar(); // I already know it is comment, so move
                                         // current char and return true
            return true;
        }
        else
            goBack();
...
bool LexicalAnalyzer::isDigit() { return currentChar >= 48 && currentChar <= 57; }
...
bool LexicalAnalyzer::isKeyword(const std::string& tmp_token) {
    return tmp_token == "class" || tmp_token == "private" || tmp_token == "public" || tmp_token == "friend" ||
        tmp_token == "return";
}
...
bool LexicalAnalyzer::isBoolean(const std::string& tmp_token) { return (tmp_token == "true" || tmp_token == "false"); }
...
Token_ptr LexicalAnalyzer::buildOperator(const char& tmp_token) {
    ...
    switch (tmp_token) {
    case '/': // it won't interfere with comments, because it is already verified
...
Token_ptr LexicalAnalyzer::getNextToken() {
    ...
    if (!newToken.get()) {
        ...
        // !nextIsClosingSingleQuote) 	nextIsChar = true;
    }
