...
struct Element {
    string type = "";
    string id = "";
    Owner owner; // in element it defines which class it refers to
};
...
struct ClassInfo {
    string classId = "";
    int uniqueId = -1;
    vector<InheritedClassInfo> inherits;
...
private:
    ...
    void buildGlobalClasses(vector<Statement_ptr>& programStatements);
    ClassInfo& getMyClassInfo(string myId, int myUniqueId);
    void addInheritedElemsId(vector<Statement_ptr>& programStatements,
        ClassDefinitionStmt* motherClassDef);
    void analyzeInheritance(ClassDefinitionStmt* classDef, ClassDefinitionStmt* motherClassDef);
    void printTrees();
    ...
    void checkOperationElement(OperationElementStmt* assignedElem,
        vector<string>& identifiersToCheck);
    void addElems(string className, string statementId, vector<string>& identifiersToCheck,
        string whoCalled);
    ...
    vector<ClassInfo> globalClasses;
