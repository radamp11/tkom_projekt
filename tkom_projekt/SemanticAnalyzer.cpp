...
SemanticAnalyzer::SemanticAnalyzer(vector<ClassInfo>& classesInfo) { this->classesInfo = classesInfo; }
...
void SemanticAnalyzer::buildGlobalScope(vector<Statement_ptr>& statements) {
    ...
    for (unsigned i = 0; i < statements.size(); ++i) {
        ...
        if (stmtName == "class ClassDefinitionStmt *") {
            ...
            GlobalElem newElem;
            newElem.type = "class";
            newElem.myClassInfo = getMyClassInfo(classDef->getName(), classDef->getUniqueId());
...
void SemanticAnalyzer::checkFunBlock(FunDefinitionStmt* funDef) {
    ...
    Owner myOwner = funDef->getOwner();
    BlockStmt* funBlock = funDef->getBlock().get();
    ClassInfo myClassScope;
    if (myOwner.ownerName != "global")
        myClassScope = getMyClassInfo(myOwner.ownerName, myOwner.ownerId);
    ...
    for (unsigned i = 0; i < funBlock->getStatements().size(); ++i) {
        ...
        if (stmtName == "class ClassDefinitionStmt *") {
            ...
            newElem.owner.ownerName = classDef->getName();
            newElem.owner.ownerId = classDef->getUniqueId();
            ...
            checkOperationElement(expectedType, initStmt->getAssignedElement().get(), localFunScope, myClassScope,
                funDef->getFunId(), "init " + initStmt->getId());
            Element newElem;
            ...
            throw SemanticException("Function: " + funDef->getFunId() +
                " - You cannot define or declare a function inside another function");
        }
            ...
            checkOperationElement(expectedType, operationElement, localFunScope, myClassScope, funDef->getFunId(),
                "assign " + assignStmt->getAssigneeId());
        }
        else if (stmtName == "class ObjectInitStmt *") {
            ObjectInitStmt* objInit = dynamic_cast<ObjectInitStmt*>(funBlock->getStatements()[i].get());
            Element newElem = checkObjectInit(objInit, localFunScope, myClassScope, funDef->getFunId());
            localFunScope.push_back(newElem);
        }
            ...
            checkOperationElement(expectedType, returnStmt->getReturnedStatement().get(), localFunScope, myClassScope,
                funDef->getFunId(), "return statement");
        }
    }
}
...
void SemanticAnalyzer::checkClassBlock(ClassDefinitionStmt* classDef) {
    ...
    ClassInfo myClassScope = getMyClassInfo(classDef->getName(), classDef->getUniqueId());
    ...
    for (unsigned i = 0; i < classBlock->getStatements().size(); ++i) {
        ...
        if (stmtName == "class ClassDefinitionStmt *") {
            ...
            if (assignedElem)
                checkOperationElement(expectedType, assignedElem, localScope, myClassScope, classDef->getName(),
                    "init " + initStmt->getId());
            Element newElem;
            newElem.type = expectedType;
            newElem.id = initStmt->getId();
            ...
            throw SemanticException("Unallowed statement in class " + classDef->getName() +
                ": you cannot refer direclty to object's attribute: " + objAttr->getObjectId() + "." +
                objAttr->getAttributeId() + " in class declaration block.");
        }
        else if (stmtName == "class ObjectMethodCallStmt *") {
            ObjectMethodCallStmt* methodCall = dynamic_cast<ObjectMethodCallStmt*>(classBlock->getStatements()[i].get());
            throw SemanticException("Unallowed statement in class " + classDef->getName() +
                ": you cannot refer direclty to object's method: " + methodCall->getObjectId() + "." +
                methodCall->getCalledMethod()->getFunId() + " in class declaration block.");
        }
        else if (stmtName == "class FunCallStmt *") {
            FunCallStmt* funCall = dynamic_cast<FunCallStmt*>(classBlock->getStatements()[i].get());
            throw SemanticException("Unallowed statement in class " + classDef->getName() +
                ": you cannot refer directly to function call: " + funCall->getFunId() +
                " in class declaration block.");
        }
        else if (stmtName == "class AssignStmt *") {
            AssignStmt* assignStmt = dynamic_cast<AssignStmt*>(classBlock->getStatements()[i].get());
            throw SemanticException("Unallowed statement in class " + classDef->getName() +
                ": assignment: " + assignStmt->getAssigneeId() +
            ...
            Element newElem = checkObjectInit(objInit, localScope, myClassScope, classDef->getName());
            localScope.push_back(newElem);
        }
        else if (stmtName == "class ReturnStmt *") {
            ReturnStmt* returnStmt = dynamic_cast<ReturnStmt*>(classBlock->getStatements()[i].get());
            throw SemanticException("Unallowed return statement in class " + classDef->getName() +
                "- you cannot return anything in class declaration block.");
        }
    }
}

void SemanticAnalyzer::checkOperationElement(string expectedType, OperationElementStmt* operationElement,
    const vector<Element>& localScope, ClassInfo myClassScope,
    string whoCalled, string stmtName) {
    ...
    if (calledValue) {
        ...
        if (expectedType != type) {
            ...
            throw SemanticException("Invalid value in: " + stmtName + " in function/class: " + whoCalled +
                ". Invalid type of " + message + " element.");
        }
    }
...
void SemanticAnalyzer::checkGetObjectAttribute(string expectedType, GetObjectAttributeStmt* objAttr,
    const vector<Element>& localFunScope, ClassInfo myClassScope,
    string whoCalled) {
    ...
    for (unsigned i = 0; i < localFunScope.size(); ++i) {
        if (objectId == localFunScope[i].id) {
            ClassInfo classScope = getMyClassInfo(localFunScope[i].type, localFunScope[i].owner.ownerId);
            if (classScope.classId == "")
                throw SemanticException("Class " + localFunScope[i].type + ", used in " + whoCalled +
                    " function/class does not exist.");
            ...
            for (unsigned j = 0; j < classScope.publicAttributes.size(); ++j) {
                if (attributeId == classScope.publicAttributes[j].id) {
                    if (expectedType == "none")
                        ...
                        throw SemanticException("Invalid type of attribute: " + classScope.publicAttributes[j].type +
                            ", expected: " + expectedType + " in function/class" + whoCalled);
                    return;
                }
            }
            for (unsigned j = 0; j < classScope.privateAttributes.size(); ++j) {
                if (attributeId == classScope.privateAttributes[j].id) {
                    if (!isFriend)
                        throw SemanticException("Cannot access private attribute: " + classScope.privateAttributes[j].id +
                            " of class " + className + " in function/class " + whoCalled);
                    if (expectedType == "none")
                        return;
                    else if (classScope.privateAttributes[j].type != expectedType)
                        throw SemanticException("Invalid type of attribute: " + classScope.privateAttributes[j].type +
                            ", expected: " + expectedType + " in function/class" + whoCalled);
                    return;
                }
            }
            for (unsigned j = 0; j < classScope.inheritedAttributes.size(); ++j) {
                if (attributeId == classScope.inheritedAttributes[j].elemId) {
                    if (expectedType == "none")
                        ...
                        throw SemanticException("Invalid type of attribute: " + classScope.inheritedAttributes[j].type +
                            ", expected: " + expectedType + " in function/class" + whoCalled);
                    return;
                }
            }
            for (unsigned j = 0; j < classScope.privateInheritedAttributes.size(); ++j) {
                if (attributeId == classScope.privateInheritedAttributes[j].elemId) {
                    if (!isFriend)
                        throw SemanticException("Cannot access private attribute: " +
                            classScope.privateInheritedAttributes[j].elemId + " in function/class" + whoCalled);
                    if (expectedType == "none")
                        return;
                    else if (classScope.privateInheritedAttributes[j].type != expectedType)
                        throw SemanticException("Invalid type of attribute " + classScope.privateInheritedAttributes[j].elemId +
                            ": " + classScope.privateInheritedAttributes[j].type +
                            ", expected: " + expectedType + " in function/class" + whoCalled);
                    return;
                }
            }
    ...
    for (unsigned j = 0; j < myClassScope.publicAttributes.size(); ++j) {
        if (attributeId == myClassScope.publicAttributes[j].id) {
            if (expectedType == "none")
                return;
            else if (myClassScope.publicAttributes[j].type != expectedType)
                throw SemanticException("Invalid type of attribute: " + myClassScope.publicAttributes[j].type +
                    ", expected: " + expectedType + " in function/class" + whoCalled);
            return;
        }
    }
    for (unsigned j = 0; j < myClassScope.privateAttributes.size(); ++j) {
        if (attributeId == myClassScope.privateAttributes[j].id) {
            if (expectedType == "none")
                return;
            else if (myClassScope.privateAttributes[j].type != expectedType)
                throw SemanticException("Invalid type of attribute: " + myClassScope.privateAttributes[j].type +
                    ", expected: " + expectedType + " in function/class" + whoCalled);
            return;
        }
    }
    for (unsigned j = 0; j < myClassScope.inheritedAttributes.size(); ++j) {
        if (attributeId == myClassScope.inheritedAttributes[j].elemId) {
            if (expectedType == "none")
                return;
            else if (myClassScope.inheritedAttributes[j].type != expectedType)
                throw SemanticException("Invalid type of attribute: " + myClassScope.inheritedAttributes[j].type +
                    ", expected: " + expectedType + " in function/class" + whoCalled);
            return;
        }
    }
    for (unsigned j = 0; j < myClassScope.privateInheritedAttributes.size(); ++j) {
        if (attributeId == myClassScope.privateInheritedAttributes[j].elemId) {
            if (expectedType == "none")
                return;
            else if (myClassScope.privateInheritedAttributes[j].type != expectedType)
                throw SemanticException("Invalid type of attribute " + myClassScope.privateInheritedAttributes[j].elemId +
                    ": " + myClassScope.privateInheritedAttributes[j].type + ", expected: " + expectedType +
                    " in function/class" + whoCalled);
            return;
        }
    }
    throw SemanticException("Undefined attribute: " + attributeId + " of class " + className +
        " used in function/class: " + whoCalled);
}

void SemanticAnalyzer::checkObjectMethodCall(string expectedType, ObjectMethodCallStmt* objMethodCall,
    const vector<Element>& localFunScope, ClassInfo myClassScope,
    string whoCalled) {
    ...
    for (unsigned i = 0; i < localFunScope.size(); ++i) {
        if (objectId == localFunScope[i].id) {
            if (localFunScope[i].owner.ownerId == -1) // it is not an object, or it is not valid
                continue;
            ClassInfo classScope = getMyClassInfo(localFunScope[i].type, localFunScope[i].owner.ownerId);
            if (classScope.classId == "")
                throw SemanticException("Class " + localFunScope[i].type + ", used in " + whoCalled +
                ...
                if (methodId == classScope.publicMethods[j].id) {
                    ...
                    if (calledArgs->getListOfArguments().size() != expectedArgCount)
                        throw SemanticException(
                            "Invalid amount of arguments in function call: " + methodId + " called in " + whoCalled +
                            " function/class. Provided: " + std::to_string(calledArgs->getListOfArguments().size()) +
                    ...
                    for (unsigned k = 0; k < classScope.publicMethods[j].arguments.size(); ++k) {
                        ...
                        checkOperationElement(expectedArgType, calledArgs->getListOfArguments()[k].get(), localFunScope,
                            myClassScope, whoCalled, "method " + methodId + " call");
                    }
                    if (expectedType == "none")
                        return;
                    else if (classScope.publicMethods[j].type != expectedType)
                        throw SemanticException("Invalid returned type: " + classScope.publicMethods[j].type + " of method: " +
                            methodId + ", expected: " + expectedType + " in function/class" + whoCalled);
                    return;
                }
            }
            for (unsigned j = 0; j < classScope.privateMethods.size(); ++j) {
                if (methodId == classScope.privateMethods[j].id) {
                    if (!isFriend)
                        throw SemanticException("Cannot access private method: " + classScope.privateMethods[j].id +
                            " in function/class" + whoCalled);
                    string expectedArgType;
                    ...
                    if (expectedType == "none")
                        ...
                        throw SemanticException("Invalid type of method " + classScope.privateMethods[j].id + ": " +
                            classScope.privateMethods[j].type + ", expected: " + expectedType +
                            " in function/class" + whoCalled);
                    return;
                }
            }
            for (unsigned j = 0; j < classScope.inheritedMethods.size(); ++j) {
                if (methodId == classScope.inheritedMethods[j].id) {
                    ...
                    if (expectedType == "none")
                        ...
                        throw SemanticException("Invalid type of method " + classScope.inheritedMethods[j].id + ": " +
                            classScope.inheritedMethods[j].type + ", expected: " + expectedType +
                            " in function/class" + whoCalled);
                    return;
                }
            }
            for (unsigned j = 0; j < classScope.privateInheritedMethods.size(); ++j) {
                if (methodId == classScope.privateInheritedMethods[j].id) {
                    if (!isFriend)
                        throw SemanticException("Cannot access private method: " + classScope.privateInheritedMethods[j].id +
                            " in function/class" + whoCalled);
                    string expectedArgType;
                    ...
                    if (expectedType == "none")
                        ...
                        throw SemanticException("Invalid type of method " + classScope.privateInheritedMethods[j].id + ": " +
                            classScope.privateInheritedMethods[j].type + ", expected: " + expectedType +
                            " in function/class" + whoCalled);
                    return;
                }
            }
        ...
        if (i == localFunScope.size() - 1)
            throw SemanticException("Undefined method: " + methodId + " of class  " + className +
                " used in function/class: " + whoCalled);
    }

    for (unsigned j = 0; j < myClassScope.publicMethods.size(); ++j) {
        if (methodId == myClassScope.publicMethods[j].id) {
            string expectedArgType;
            int expectedArgCount = myClassScope.publicMethods[j].arguments.size();
            if (calledArgs->getListOfArguments().size() != expectedArgCount)
                throw SemanticException(
                    "Invalid amount of arguments in function call: " + methodId + " called in " + whoCalled +
                    " function/class. Provided: " + std::to_string(calledArgs->getListOfArguments().size()) +
                    ", expected: " + std::to_string(expectedArgCount));
            for (unsigned k = 0; k < myClassScope.publicMethods[j].arguments.size(); ++k) {
                expectedArgType = myClassScope.publicMethods[j].arguments[k].type;
                checkOperationElement(expectedArgType, calledArgs->getListOfArguments()[k].get(), localFunScope, myClassScope,
                    whoCalled, "method " + methodId + " call");
            }
            if (expectedType == "none")
                return;
            else if (myClassScope.publicMethods[j].type != expectedType)
                throw SemanticException("Invalid returned type: " + myClassScope.publicMethods[j].type + " of method: " +
                    methodId + ", expected: " + expectedType + " in function/class" + whoCalled);
            return;
        }
    }
    for (unsigned j = 0; j < myClassScope.privateMethods.size(); ++j) {
        if (methodId == myClassScope.privateMethods[j].id) {
            string expectedArgType;
            int expectedArgCount = myClassScope.privateMethods[j].arguments.size();
            if (calledArgs->getListOfArguments().size() != expectedArgCount)
                throw SemanticException(
                    "Invalid amount of arguments in function call: " + methodId + " called in " + whoCalled +
                    " function/class. Provided: " + std::to_string(calledArgs->getListOfArguments().size()) +
                    ", expected: " + std::to_string(expectedArgCount));
            for (unsigned k = 0; k < myClassScope.privateMethods[j].arguments.size(); ++k) {
                expectedArgType = myClassScope.privateMethods[j].arguments[k].type;
                checkOperationElement(expectedArgType, calledArgs->getListOfArguments()[k].get(), localFunScope, myClassScope,
                    whoCalled, "method " + methodId + " call");
            }
            if (expectedType == "none")
                return;
            else if (myClassScope.privateMethods[j].type != expectedType)
                throw SemanticException("Invalid type of method " + myClassScope.privateMethods[j].id + ": " +
                    myClassScope.privateMethods[j].type + ", expected: " + expectedType +
                    " in function/class" + whoCalled);
            return;
        }
    }
    for (unsigned j = 0; j < myClassScope.inheritedMethods.size(); ++j) {
        if (methodId == myClassScope.inheritedMethods[j].id) {
            string expectedArgType;
            int expectedArgCount = myClassScope.inheritedMethods[j].arguments.size();
            if (calledArgs->getListOfArguments().size() != expectedArgCount)
                throw SemanticException(
                    "Invalid amount of arguments in function call: " + methodId + " called in " + whoCalled +
                    " function/class. Provided: " + std::to_string(calledArgs->getListOfArguments().size()) +
                    ", expected: " + std::to_string(expectedArgCount));
            for (unsigned k = 0; k < myClassScope.inheritedMethods[j].arguments.size(); ++k) {
                expectedArgType = myClassScope.inheritedMethods[j].arguments[k].type;
                checkOperationElement(expectedArgType, calledArgs->getListOfArguments()[k].get(), localFunScope, myClassScope,
                    whoCalled, "method " + methodId + " call");
            }
            if (expectedType == "none")
                return;
            else if (myClassScope.inheritedMethods[j].type != expectedType)
                throw SemanticException("Invalid type of method " + myClassScope.inheritedMethods[j].id + ": " +
                    myClassScope.inheritedMethods[j].type + ", expected: " + expectedType +
                    " in function/class" + whoCalled);
            return;
        }
    }
    for (unsigned j = 0; j < myClassScope.privateInheritedMethods.size(); ++j) {
        if (methodId == myClassScope.privateInheritedMethods[j].id) {
            string expectedArgType;
            int expectedArgCount = myClassScope.privateInheritedMethods[j].arguments.size();
            if (calledArgs->getListOfArguments().size() != expectedArgCount)
                throw SemanticException(
                    "Invalid amount of arguments in function call: " + methodId + " called in " + whoCalled +
                    " function/class. Provided: " + std::to_string(calledArgs->getListOfArguments().size()) +
                    ", expected: " + std::to_string(expectedArgCount));
            for (unsigned k = 0; k < myClassScope.privateInheritedMethods[j].arguments.size(); ++k) {
                expectedArgType = myClassScope.privateInheritedMethods[j].arguments[k].type;
                checkOperationElement(expectedArgType, calledArgs->getListOfArguments()[k].get(), localFunScope, myClassScope,
                    whoCalled, "method " + methodId + " call");
            }
            if (expectedType == "none")
                return;
            else if (myClassScope.privateInheritedMethods[j].type != expectedType)
                throw SemanticException("Invalid type of method " + myClassScope.privateInheritedMethods[j].id + ": " +
                    myClassScope.privateInheritedMethods[j].type + ", expected: " + expectedType +
                    " in function/class" + whoCalled);
            return;
        }
    }
...
void SemanticAnalyzer::checkFunCall(string expectedType, FunCallStmt* funCall, const vector<Element>& localFunScope,
    ClassInfo myClassScope, string whoCalled) {
        ...
        if (globalElements[i].type == "function") {
            if (funId == globalElements[i].myFunInfo.id) {
                ...
                if (funCallArgs->getListOfArguments().size() != expectedArgCount)
                    throw SemanticException(
                        "Invalid amount of arguments in function call: " + funId + " called in " + whoCalled +
                        " function/class. Provided: " + std::to_string(funCallArgs->getListOfArguments().size()) +
                        ", expected: " + std::to_string(expectedArgCount));
                for (unsigned j = 0; j < expectedArgCount; ++j) {
                    expectedArgType = globalElements[i].myFunInfo.arguments[j].type;
                    checkOperationElement(expectedArgType, funCallArgs->getListOfArguments()[j].get(), localFunScope,
                        myClassScope, whoCalled, "function " + funId);
                }
                if (expectedType == "none")
                    return;
...
string SemanticAnalyzer::checkAssignStmt(AssignStmt* assignStmt, const vector<Element>& localFunScope,
    ClassInfo myClassScope, string whoCalled) {
    string type;
...
Element SemanticAnalyzer::checkObjectInit(ObjectInitStmt* objInit, const vector<Element>& localFunScope,
    ClassInfo myClassScope, string whoCalled) {
    ...
    for (unsigned i = 0; i < localFunScope.size(); ++i) {
        if (className == localFunScope[i].type &&
            className == localFunScope[i].id) { // by my convention classes in local scopes are stored like this
            found.type = className;
            found.id = objectId;
            found.owner = localFunScope[i].owner; // co jest ownerem? ta klasa czy co?
            return found;
        }
    }
    found = findAttributeInClassScope(className, className, myClassScope, whoCalled);
    if (found.type != "") {
        found.type = className;
        found.id = objectId;
        return found;
    ...
    for (unsigned i = 0; i < globalElements.size(); ++i) {
        if (globalElements[i].type == "class" && globalElements[i].myClassInfo.classId == className) {
            found.type = className;
            found.id = objectId;
            found.owner.ownerName = className;
            found.owner.ownerId = globalElements[i].myClassInfo.uniqueId;
            return found;
        }
    }
    throw SemanticException("Undefined class: " + className + " in function/class " + whoCalled + "'s scope.");
}

Element SemanticAnalyzer::checkIdentifier(string expectedType, string calledAttribute,
    const vector<Element>& localFunScope, ClassInfo myClassScope,
    string whoCalled, string stmtName) {
    Element found;
    for (unsigned i = 0; i < localFunScope.size(); ++i) {
        if (calledAttribute == localFunScope[i].id) {
            ...
            throw SemanticException("Invalid type of identifier: " + calledAttribute + " used in " + stmtName + " in class " +
                myClassScope.classId + " in " + whoCalled + " method.");
        }
    }
...
Element SemanticAnalyzer::findAttributeInClassScope(string expectedType, string calledAttribute, ClassInfo myClassScope,
    string whoCalled) {
    Element found;
    for (unsigned i = 0; i < myClassScope.privateAttributes.size(); ++i) {
        if (calledAttribute == myClassScope.privateAttributes[i].id) {
            ...
            found.owner = myClassScope.privateAttributes[i].owner;
            if (expectedType == "none") {
                found.type = "none";
                return found;
            }
    ...
    for (unsigned i = 0; i < myClassScope.publicAttributes.size(); ++i) {
        if (calledAttribute == myClassScope.publicAttributes[i].id) {
            found.id = calledAttribute;
            found.owner = myClassScope.publicAttributes[i].owner;
            if (expectedType == "none") {
                found.type = "none";
                return found;
            }
    ...
    for (unsigned i = 0; i < myClassScope.inheritedAttributes.size(); ++i) {
        if (calledAttribute == myClassScope.inheritedAttributes[i].elemId) {
            found.id = calledAttribute;
            found.owner = myClassScope.inheritedAttributes[i].owner;
            if (expectedType == "none") {
                found.type = "none";
                return found;
            }
...
string SemanticAnalyzer::findIdentifiersType(string identifier, const vector<Element>& localFunScope,
    ClassInfo myClassScope, string whoCalled) {
    string found;
    for (unsigned i = 0; i < localFunScope.size(); ++i) {
...
bool SemanticAnalyzer::findMethodInClassScope(CallArgsStmt* funCallArgs, string expectedType, string calledMethod,
    ClassInfo myClassScope, string whoCalled) {
    ...
    for (unsigned i = 0; i < myClassScope.privateMethods.size(); ++i) {
        if (calledMethod == myClassScope.privateMethods[i].id) {
            ...
            if (funCallArgs->getListOfArguments().size() != expectedArgCount)
                throw SemanticException(
                    "Invalid amount of arguments in function call: " + calledMethod + " called in " + whoCalled +
                    " function/class. Provided: " + std::to_string(funCallArgs->getListOfArguments().size()) +
                    ", expected: " + std::to_string(expectedArgCount));
            for (unsigned j = 0; j < myClassScope.privateMethods[i].arguments.size(); ++j) {
                ...
                checkOperationElement(expectedArgType, funCallArgs->getListOfArguments()[j].get(), emptyLocalScope,
                    myClassScope, whoCalled, "method " + calledMethod + " call");
            }
            if (expectedType == "none")
                return true;
...
void SemanticAnalyzer::addArgsToLocalFunScope(FunDefinitionStmt* funDef, vector<Element>& localFunScope) {
    ...
    for (unsigned i = 0; i < funArgs->getSignatures().size(); ++i) {
        ...
        if (stmtName == "class ClassSignatureStmt *") {
            ...
            newElem.owner.ownerName = classSig->getMyClassName();
            if (funDef->getOwner().ownerName == "global")
                for (unsigned i = 0; i < globalElements.size(); ++i) {
                    if (classSig->getMyClassName() == globalElements[i].myClassInfo.classId)
                        newElem.owner.ownerId = globalElements[i].myClassInfo.uniqueId;
                }
            else {
                ClassInfo myClassInfo =
                    getMyClassInfo(funDef->getOwner().ownerName, funDef->getOwner().ownerId); // look in local scope
                newElem = findAttributeInClassScope(classSig->getMyClassName(), classSig->getMyClassName(), myClassInfo,
                    funDef->getFunId());
                if (newElem.id == "") { // if not in local, search global
                    newElem.type = classSig->getMyClassName();
                    newElem.id = classSig->getMyId();
                    newElem.owner.ownerName = classSig->getMyClassName();
                    for (unsigned i = 0; i < globalElements.size(); ++i) {
            ...
            if (newElem.owner.ownerId == -1)
                throw SemanticException("Cannot access class " + classSig->getMyClassName() + " in fun/method" +
                    funDef->getFunId() + "'s scope.");
        }
        else if (stmtName == "class SignatureStmt *") {
...
ClassInfo SemanticAnalyzer::getMyClassInfo(string myId, int myUniqueId) {
    for (unsigned i = 0; i < classesInfo.size(); ++i) {
        if (classesInfo[i].classId == myId && classesInfo[i].uniqueId == myUniqueId)
            return classesInfo[i];
    }
    std::cerr << "Semantic analysis: No such a class defined" << std::endl;
    ClassInfo info;
